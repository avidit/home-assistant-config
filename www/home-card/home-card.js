...
class HomeCard extends LitElement {
    ...
    render() {
        try {
            ...
            return html`<ha-card><div class="error-message">${error}</div></ha-card>`;
        }
    }
    ...
    make_content() {
        ...
        var house = html`
            <div id="house">
                <img id="house-image" src="${this.imgPath('house', this.theme.house)}"/>
                ${this.make_entities()}
            </div>`;

        if (this.config.weather) {
            var weatherObj = this.stateObject("weather", this.config.weather);
            var state = weatherObj.state
            var img = `${this.imgPath('weather', `${state}.png`)}`
            weather = html`
                <div id="weather">
                    <img id="weather-icon" src="${img}"/>
                    <span id="weather-info">
                        ${state}
                        ${weatherObj.attributes.temperature}
    ...
    create_overlay(imageName, imageFile, style, entity) {
        ...
        var img = this.imgPath('overlays', imageFile)
        return html`
            <img
                id="${imageName}"
                src="${img}"
                class="element"
                style="${styleString}"
                @mousedown="${() => this._down(entity)}"
                @touchstart="${() => this._down(entity)}"
                @mouseup="${() => this._up(entity, true)}"
            />
            `;
    }

    imgPath(folder, filename) {
        return `/local/home-card/resources/${folder}/${filename}?v=${VERSION}`;
    }
